...
//    private SearchPipelineScript searchScript;
    ...
    // Test that the script processor can modify the search request query parameters
    public void testScriptModifyQueryParameters() throws Exception {
        String scriptName = "test-search-script";
        ScriptService scriptService = new ScriptService(
            Settings.builder().build(),
            Collections.singletonMap(
        ...
        searchSourceBuilder.query(QueryBuilders.queryStringQuery("foo"));
        SearchRequest searchRequest = new SearchRequest().source(searchSourceBuilder);
        // add a query to the search request
        scriptProcessor.processRequest(searchRequest);
